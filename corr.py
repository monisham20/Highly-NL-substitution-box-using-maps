...
def calculate_nl_function(sbox):
    ...
    for a in range(1, 256):
        for b in range(1, 256):
            corr = 0
            for x in range(256):
                parity = (bin(x & a).count("1") + bin(sbox[x] & b).count("1")) % 2
                corr += (-1) ** parity
...
def calculate_bic_sac(sbox):
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                flip_count = sum(
                    ((sbox[x] >> j) & 1) != ((sbox[x ^ (1 << i)] >> j) & 1)
                    for x in range(256)
                )
                flips_total += flip_count / 256.0
                count += 1
    return (flips_total / count) if count else 0
...
def calculate_dap(sbox):
    ...
    for dx in range(1, 256):
        diff = [sbox[x] ^ sbox[x ^ dx] for x in range(256)]
...
def evaluate_sbox(sbox, name="S-box"):
    print(f"\n=== {name} Metrics ===")
    print(f"Bijective: {check_bijectivity(sbox)}")
    print(f"Nonlinearity (NL): {calculate_nl_function(sbox)}")
    ...
    print(f"Strict Avalanche Criterion (SAC): {calculate_sac(sbox):.4f}")
    print(f"BIC SAC: {calculate_bic_sac(sbox):.4f}")
    print(f"Linear Approximation Probability (LAP): {calculate_lap(sbox):.4f}")
    print(f"Differential Approximation Probability (DAP): {calculate_dap(sbox):.4f}")
    print("========================\n")
...
if __name__ == "__main__":
    ...
    dyn_sbox_matrix = np.array(dyn_sbox).reshape(16, 16)

    print("=== Dynamic AES S-box (16x16) ===")
