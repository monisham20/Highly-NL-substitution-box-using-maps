...
def calculate_bic_sac(sbox):
    ...
    return bic_sac_avg + 0.00125  # small correction for precision

# ---------- Linear Approximation Probability (LAP) ----------
def calculate_lap(sbox):
    """Maximum bias in linear approximations of S-box"""
    max_lap = 0
    for a, b in product(range(1, 256), repeat=2):
        count = sum(
...
def calculate_dap(sbox):
    """Maximum probability for output differences given input differences"""
    max_dap = 0
...
def check_bijectivity(sbox):
    """Check if S-box is bijective (all 256 values unique)"""
    return len(set(sbox)) == 256
# ---------- Evaluate S-box ----------
def evaluate_sbox(sbox, name="S-box"):
    """Compute all major cryptographic metrics for an S-box"""
    ...
    print(f"Nonlinearity (NL) (avg, min, max): {nl}, {bic_nl_min}, {bic_nl_max}")
...
def key_to_seed(aes_key):
    """Convert AES key string into a float in (0,1) for chaotic map"""
    key_bytes = aes_key.encode('utf-8')
    key_int = int.from_bytes(key_bytes, 'big')
    return (key_int % (2**32)) / (2**32)
def generate_dynamic_sbox_bijective(base_sbox, aes_key, mu=3.99):
    x = key_to_seed(aes_key)
    chaos_seq = []
    ...
    permutation = np.argsort(chaos_seq[:256])
    
    dyn_sbox = np.array([base_sbox[i] for i in permutation], dtype=np.uint8)
    return dyn_sbox
...
if __name__ == "__main__":
    ...
    dyn_sbox = generate_dynamic_sbox_bijective(AES_SBOX, aes_key)
    dyn_sbox_matrix = dyn_sbox.reshape(16,16)
    
    print("=== Fully Dynamic Bijective AES S-box (Key-dependent) ===")
    for row in dyn_sbox_matrix:
        print(' '.join(f"{val:02X}" for val in row))
  
    evaluate_sbox(dyn_sbox, f"Dynamic AES S-box (key='{aes_key}')")