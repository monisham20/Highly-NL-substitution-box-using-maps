...
def calculate_nl_function(sbox):
    ...
    for bit in range(n):
        ...
        nl_list.append(calculate_nonlinearity(f))
    return min(nl_list), int(np.mean(nl_list)), max(nl_list)
            ...
            if i != j:
                ...
                for x in range(256):
                    ...
                    if bit_output != bit_output_flipped:
                        flip_count += 1
                bic_sac_sum += flip_count / 256.0
                count += 1
    return bic_sac_sum / count

# ---------- Linear Approximation Probability (LAP) ----------
def calculate_lap(sbox):
    max_lap = 0
    for a, b in product(range(1, 256), repeat=2):
        count = sum(1 for x in range(256) if hamming_weight((x & a) ^ (sbox[x] & b)) % 2 == 0)
...
def evaluate_sbox(sbox, name="S-box"):
    nl_min, nl_avg, nl_max = calculate_nl_function(sbox)
    ...
    print(f"Nonlinearity (NL) (min, avg, max): {nl_min}, {nl_avg}, {nl_max}")
    print(f"BIC Nonlinearity: {bic_nl_avg}")
    print(f"Strict Avalanche Criterion (SAC): {sac:.4f}")
    print(f"BIC SAC: {bic_sac:.4f}")
    print(f"Linear Approximation Probability (LAP): {lap:.4f}")
    print(f"Differential Approximation Probability (DAP): {dap:.4f}")
    print("========================\n")

# ---------- Key to Seed ----------
def key_to_seed(aes_key):
    key_bytes = aes_key.encode('utf-8')
    key_int = int.from_bytes(key_bytes, 'big')
    return (key_int % (2**32)) / (2**32)

# ---------- Generate Strong Dynamic S-box with Target Metrics ----------
def generate_strong_sbox(aes_key, mu=3.99, target_nl=112, max_iter=1000):
    AES_SBOX = np.array([
    ...
    for _ in range(max_iter):
        ...
        perm = np.argsort(chaos_seq[:256])
        sbox = np.array([AES_SBOX[i] for i in perm], dtype=np.uint8)
        
        # Simple affine transformation
        sbox = np.array([((v << 1) & 0xFF) ^ ((v >> 7) & 1) ^ 0xA5 for v in sbox], dtype=np.uint8)
        
        # Ensure bijective
        sbox = np.array(np.argsort(np.argsort(sbox)), dtype=np.uint8)
        
        nl_min, nl_avg, nl_max = calculate_nl_function(sbox)
        if nl_avg >= target_nl:
            return sbox
    
    return sbox  # fallback if target not exactly reached

# ================== Main ==================
if __name__ == "__main__":
    key = "thisis128bitkey"
    strong_sbox = generate_strong_sbox(key)
    sbox_matrix = strong_sbox.reshape(16,16)
    
    print("=== Strong Dynamic AES S-box (Key-dependent) ===")
    for row in sbox_matrix:
        print(' '.join(f"{val:02X}" for val in row))
    
    evaluate_sbox(strong_sbox, f"Strong AES S-box (key='{key}')")
